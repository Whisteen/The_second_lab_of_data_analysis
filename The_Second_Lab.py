...
# Программу осознано старался делать без исключений и их обработки, чтобы не нарушать логику работы.
...
for i in range(count_i):
    ...
    for j in range(count_j):
        ...
        # Цикл для прохода по таблице, парсинга и редактирования нужных данных (Через Pansas не вышло - пришлось искать другой метод с помощью xpath, например)
        # Большие количество проверок и условий сделано по причине разной верстки разделов сайта.
        # Вызод из цикла сразу после нахождения требуемых данных, чтобы не тратить время и ресурсы попусту
